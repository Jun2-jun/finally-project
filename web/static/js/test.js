document.addEventListener('DOMContentLoaded', () => {
    ...
    // 1. 현재 사용자 정보 가져오기
    fetch('http://192.168.219.84:5002/api/current-user', {
      method: 'GET',
      credentials: 'include'
    })
      .then(res => res.json())
      .then(userData => {
        if (userData.status === 'success' && userData.user) {
          ...
          // 2. 예약정보 가져오기
          return fetch(`http://192.168.219.84:5002/api/reservations/user/${userId}`, {
            method: 'GET',
            credentials: 'include'
          });
        } else {
        ...
        if (reservationData.status === 'success' && reservationData.data.length > 0) {
          reservationData.data.forEach((r, i) => {
            ...
            if (reservationTime < now) {
              // 과거 예약 삭제
              fetch(`http://192.168.219.84:5002/api/mypage/reservation/${r.id}`, {
                method: 'DELETE',
                credentials: 'include'
              })
              .then(res => {
        ...
        document.querySelectorAll(".cancel-btn").forEach(button => {
          button.addEventListener("click", event => {
            ...
            if (confirm("정말 이 예약을 취소하시겠습니까?")) {
              fetch(`http://192.168.219.84:5002/api/mypage/reservation/${reservationId}`, {
                method: "DELETE",
                credentials: "include"
              })
                .then(res => {
    ...
    // 3. 민감정보 조회
    fetch('http://192.168.219.84:5002/api/patient/info', {
      method: 'GET',
      credentials: 'include'
    })
      .then(res => res.json())
